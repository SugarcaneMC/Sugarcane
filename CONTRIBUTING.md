...
* [Configuration files](#configuration-files)
  * [SugarcaneConfig example](#sugarcaneconfig-example)
...
Sugarcane will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Sugarcane from
...
- `Sugarcane-API` - Modifications to the API;
- `Sugarcane-Server` - Modifications to the server.
...
3. cd into `Sugarcane-Server` for server changes, and `Sugarcane-API` for API changes.  

`Sugarcane-Server` and `Sugarcane-API` aren't git repositories in the traditional sense:

- `base` points to the unmodified source before Sugarcane patches have been applied.
- Each commit after `base` is a patch.

## Adding Patches

Adding patches to Sugarcane is very simple:

1. Modify `Sugarcane-Server` and/or `Sugarcane-API` with the appropriate changes;
...
Your commit will be converted into a patch that you can then PR into Sugarcane.
...
1. If your PR creates new patches instead of modifying exist ones, in both the `Sugarcane-Server` and `Sugarcane-API` directories, ensure your newly-created patch is the last commit by either:
...
Remember that these changes will affect everyone who runs Sugarcane, not just you
and your server.
...
- Multi-line changes start with `// Sugarcane start` and end with `// Sugarcane end`;
- You can always put a comment with an explanation for better readability, like this:
`// Sugarcane start - reason`.
   - The comments should generally be about the reason the change was made, what
  it was before, or what the change is.
   - Multi-line messages should start with `// Sugarcane start` and use `/* Multi
...
When submitting patches to Sugarcane, we may ask you to add notes to the patch
...
If you add a message to your commit in the `Sugarcane-Server`/`Sugarcane-API`
...
to make future updates easier on ourselves, Sugarcane tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Sugarcane - OBFHELPER
// ...   
this.someMethod(fromX); // Sugarcane
```
...
`SugarcaneConfig` or `SugarcaneWorldConfig` classes. Use `SugarcaneConfig` if a value
...
```java
if (!SugarcaneConfig.saveEmptyScoreboardTeams) {
```

It is often preferred that you use the fully qualified name for the
configuration class when accessing it, like so:
`org.sugarcanemc.sugarcane.config.SugarcaneConfig.valueHere`.  
If this is not done, a developer for Sugarcane might fix that for you before
...
and additions. To use the test plugin, clone the Paper repo and enable it in `test-plugin.settings.gradle.kts`,
which will be generated after running Gradle in the Paper repo at least once. After this, you can edit
the test plugin, copy it to the Sugarcane repo, and run a server with the plugin using `./gradlew runDev` (or any
of the other Sugarcane run tasks).

### Publishing to Maven local (use in external plugins)

To build and install the Sugarcane APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Sugarcane repository added, put the local repository above Sugarcane's.
...
- If you later need to use the Sugarcane-API, you might want to remove the jar
...
By default, Sugarcane (and upstream) only import files we make changes to. If you
would like to make changes to a file that isn't present in `Sugarcane-Server`'s
...
1. Identify the name(s) of the file(s) you want to import.
   ...
   `./Sugarcane-Server/.gradle/caches/paperweight/mc-dev-sources/net/minecraft/`. You might find
